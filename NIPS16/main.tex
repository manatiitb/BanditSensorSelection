...
\title{Unsupervised Cost-Sensitive Online Prediction}
...
We thus make an assumption on the observed feedback. Specifically, we assume that if a cheap sensor predicts a label correctly, the prediction of a better quality sensor is also correct. We refer to this assumption as {\em dominance condition}. When the dominance conditions holds, the learner can partially infer the losses from the stages, which, as discussed later, is sufficient to learn the best stage for a given sample. We further demonstrate that under any weaker condition the learner cannot identify the best stage. The examples discussed in the beginning satisfy dominance conditions. In the wireless communication example discussed above, if an error correcting code (ex. Reed-Solomon, LDPC \cite{Book_InferenceLearning_MacKay} recovers information in a channel with certain noise level, then certainly we can recover the information on the channel with more redundancy blocks in the error correction code (though at a lower transmission rate).     
...
can be immediately applied to achieve new results
for SAP. Although the underlying
reduction is straightforward, it provides a complete characterization of the fundamental limits and what can be achieved for the SAP.
