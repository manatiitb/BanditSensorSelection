...
%		\STATE Set $n_e(t+1)=n_e(t)$.
	\ELSE
		\IF{$\min_{i\in \iset{K}}m_i(N(t-1))<\beta(n_e)/K$} \label{alg:starve}
...
Since cost vector $c$ is fixed, in the following we use parameter $\gamma$ in place $\theta$ to denote the problem instance.
