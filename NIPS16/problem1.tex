...
\todom{How about moving this to footnote?}Note that even if $i<j$ are optimal actions, there can be suboptimal actions in the interval $[i,j] (=[i,j]\cap \N)$
...
is larger than the marginal decrease in error, $\gamma_i - \gamma_j$:
