...
Let $\P$ be the set of SAPs with action set $\A = [K]$.
The corresponding bandit problems will have the same action set, for action $k\in [K]$,
the neighborhood set is $N(k) = [k]$.
Now take any instance $(P,c)\in \P$ and let $(Y,Y^1,\dots,Y^K) \sim P$.
Then we let the reward distribution for arm $k$ a shifted Bernoulli distribution.
In particular, the reward for arm $k$ follows the distribution of 
$-\one{Y^k\ne Y^1} - (c_1+\dots+c_k)$.
The rewards for different arms are defined to be independent of each other.
\todo[inline]{
