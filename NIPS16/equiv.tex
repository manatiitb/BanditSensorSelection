...
In SAP with two actions, feedback from action $1$ reveals no information about the loss incurred in a round. However feedback from action $2$ reveals (partial) information about the loss of both actions. To see this, let action $2$ is played in in round $t$. If predictions of sensors disagree, i.e., ${\hat{Y}^1_t\neq\hat{Y}^2_t}$, then due to dominance property the only possibility is $\hat{Y}^1_t \neq Y_t$ and $\hat{Y}^2_t=Y_t$. Hence $L_t(1)=1$ and $L_t(2)=c$. On the other hand, if predictions agree, i.e., ${\hat{Y}^1_t = \hat{Y}^2_t}$, then either predictions of both are correct or both are incorrect, hence $L_t(2)-L_t(1)=c_1>0$. Thus, each time learner plays action $2$, he gets to know loss of each action only in some instances. This sounds similar to the standard apple tasting problem \cite{IC2000_AppleTasting_HelmboldLittlestoneLong}
]. 

\noindent
{\bf Apple tasting :} In this problem, a learner gets a sequence of apples and some of them can be rotten. In each round, the learner can either accept or reject an apple. If apple is rejected, learner incurs a penalty if it is rotten, but do not get to observe its quality. If an apple is accepted, the learner tastes it and incurs a penalty if it is rotten. In the latter case, learner observers quality of apple, hence knows loss incurred, and also the loss he would have incurred if it was rejected.  %Thus, each time the learner accepts an apple, he knows the penalty incurred for both the actions. However, no loss values is revealed if he rejects. 
The goal of the learner is to taste more good apples. 
A SAP with dominance property is thus a more general version than the apple tasting problem as unlike in apple tasting problem only partial information on loss value are revealed.  We next show that SAP satisfying dominance property can be efficiently solved.  
%
%\begin{thm}
...
\end{equation}
\end{proposition}
This follows as one can obtain a policy for SAP from UCB-LP policy that uses side-observation graph $G^S$ as discussed in the proof of Theorem \ref{thm:K-SAPRegret}, and the new policy maintains regret guarantee of UCB-LP.   
