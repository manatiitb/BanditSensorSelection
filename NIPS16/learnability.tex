...
We now relate weak dominance to the optimality condition described in Eq.~\ref{eqn:interp_opt}. Weak dominance can be viewed as a more stringent condition for optimal actions. Namely, for action to be optimal we also require that the marginal cost be larger than marginal absolute error:
...
\end{equation}
The difference between marginal error in Eq~\ref{eqn:interp_opt} and marginal absolute error is terms of the absolute value. We will show later that weak-dominant set is a maximal learnable set, namely, the set cannot be expanded while ensuring learnability.
