...
\subsection{Algorithm for SAP with Weak Dominance}
For any suboptimal arm $j < i^*$,  $C_{i^*}-C_j \leq \P\{Y^{i^*} \neq Y^j\}$ holds, and when the WD property holds, we further have $C_j -C_{i^*} \geq \P\{Y^i \neq Y^j\}$ for all $j>i^*$. Thus, given the disagreement probabilities $\P\{Y^i \neq Y^j\}$ for all $i \neq j$, the set $\{i \in [K-1]: C_j - C_i \geq \P\{Y^i \neq Y^j\} \mbox{ for all } j>i\}$ includes the optimal arm. We use this idea in Algorithm \ref{alg:UCB} to identify the optimal arm when an instance of SAP satisfies the WD property. We will experimentally validate its performance  on real datasets in the next sections.
\begin{minipage}{0.48\textwidth}
		\begin{algorithm}[H]
