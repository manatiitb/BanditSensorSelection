...
\newcommand{\defeq}{\defeq} % triangleeq
...
%$\theta\defeq(\theta_1,\dots,\theta_K)$ of distributions over $[0,1]$ and setting $p(\cdot;a,\theta) = \theta_a(\cdot)$.
...
\end{equation}
where $\gamma_k\defeq\Pr\{Y_t\neq \hat{Y}^k_t\}$ is the misclassification error rate of sensor $k$ and $C_k \defeq c_1+\dots+c_k$ denotes the total cost for selecting the first $k$ sensors.
...
\caption{\footnotesize Depicts approximate Strong Dominance on real datasets: $\gamma_1 \defeq \Pr(Y^1 \neq Y),\,\gamma_2\defeq \Pr(Y^2 \neq Y),\, \delta_{12} \defeq \Pr(Y^1=Y,\, Y^2\neq Y)$ }
\end{center}
\vspace{-13pt}
\end{table}
...
%From the previous section, we know that given a context $x$, action $1$ is optimal if $\gamma_1(x)-\gamma_2(x) -c< 0$, otherwise  action $2$ is optimal. Let $\theta\defeq\theta_1-\theta_2$, then it boils down to check if $x^\prime\theta-c<0$ for each context $x\in \mathcal{C}$. 
%
